...
/*
MAIN GOAL: CREATE A READABLE AND UNDERSTANDABLE COMMAND STRING

Command String format: Value of 00 to 99 for motor values, as well as a 0/1 toggle for claw activation. 00 would be completely off, and 99 would be completely on (full power).

    ":"       [00-99][00-99][00-99][00-99][00-99][00-99][00-99] [00-99]  [00-99]   [00-99]   [0-1]    "\n"
verification  motor1 motor2 motor3 motor4 motor5 motor6 motor7 shoulder wristTilt wristTwist claw  endCharacter

Motor order-
1&2: Propulsion 
3: Vertical 
4-7: Quad-directional

Reasoning: The motors should be first, with propulsion coming very first. This is because we do not want to lose track of the blue lines.
After that would be the vertical shift, and then the four other motors to make smaller adjustments. 
The arm will follow, allowing it to be positioned before the claw is engaged.

Example Command String:

string data = ":505010101000009900501\n";

Breakdown:

verification = ':' This is just to authenticate with the arduino

motor1 = 50 	Sets the left propulsion to half power
motor2 = 50 	Sets the right propulsion to half power
motor3 = 10 	Slightly (1/10th) powers the upwards motor
motor4 = 10 	Slightly (1/10th) powers the front left quad motor
motor5 = 10 	Slightly (1/10th) powers the back left quad motor
motor6 = 00 	No power to the front right quad motor
motor7 = 00 	No power to the back right quad motor

shoulder = 99 	Fully rotates the shoulder joint
wristTilt = 00	Sets the wrist's tilt to the neutral position
wristTwist = 50	Twists the wrist halfway
claw = 1	Actuates the claw 

This command string would move the ROV forward while adjusting upwards and turning slightly to the left. 
After that, it would rotate the shoulder fully around, twist the wrist halfway, and grab with the claw.

We would expect the arduino to return either:

"505010101000009900501\n" 
or
"W\n"

Depending on if it detects a water leak. 

------------------------------------------------------------------------------------------------------------------


CAMERA DATA: (+/-x, +/-y, +/-z) + (optional (x,y,z) pairs)

x value (-9 to +9) would be the horizontal change away from center
y value (-9 to +9) would be the vertical change away from center
z value (-9 to +9) would be the zoom (forward/back) change away from center

(+/-x,+/-y, +z) pairs mark an objects location
The ROV will start at z = 0 and we will update its position by multiplying the motor speed by the amount of time.
This will essentially give us a complete 3D map of where each object was in the pool (at least if the ROV travels straight forward).
